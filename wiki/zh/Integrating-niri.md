...
默认配置位置可以通过 `NIRI_CONFIG` 环境变量覆盖。

<sup>Since: 26.04</sup> 你也可以在运行时通过 niri IPC 或使用命令 `niri msg action load-config-file --path <path-to-config.kdl>` 更改配置路径。

<sup>Since: 25.11</sup> 你可以通过 [`include`](./Configuration:-Include.md) 将 niri 配置文件拆分为多个文件。
...
默认的 [niri.service](https://github.com/niri-wm/niri/blob/main/resources/niri.service) 会启动 `graphical-session.target` 以及 `xdg-desktop-autostart.target`。
...
请参阅 [安全模型](./Security-Model.md) 页面了解 niri 安全模型的概述。
